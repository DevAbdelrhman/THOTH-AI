...
import (
	...
	"github.com/THOTH-AI/THOTH-Chat/backend/internal/shared/requestmeta"
)

func TestBuildVerificationEmailMessageEncodesChineseSubject(t *testing.T) {
	message := buildVerificationEmailMessage("THOTH Chat <no-reply@example.com>", "user@example.com", "123456", verificationEmailTemplate{
		Subject:      "THOTH Chat 验证码",
		Title:        "完成邮箱注册",
		SecurityNote: "如果不是您本人操作，请忽略这封邮件。",
	}, "https://thoth.example/logo.svg")

	if !strings.Contains(message, "Subject: =?utf-8?") {
		t.Fatalf("expected encoded utf-8 subject, got:\n%s", message)
	}
	if strings.Contains(message, "Subject: THOTH Chat 验证码") {
	...
	if !strings.Contains(message, `src="https://thoth.example/logo.svg"`) {
...
func TestValidateEmailRegistrationPolicy(t *testing.T) {
	cfg := config.Config{
		EmailRegistrationDomains: "example.com, @thoth-chat.ai\ncorp.cn",
		EmailRegistrationNoAlias: true,
	}

	if err := validateEmailRegistrationPolicy(cfg, "user@thoth-chat.ai"); err != nil {
		t.Fatalf("expected thoth-chat.ai to pass, got %v", err)
	}
	if err := validateEmailRegistrationPolicy(cfg, "user+alias@thoth-chat.ai"); err == nil || err.Error() != "email aliases are not allowed" {
