...
func TestBuildOTPAuthURLPreservesIssuerAccountSeparator(t *testing.T) {
	got := buildOTPAuthURL("THOTH Chat", "user@example.com", "ABCDEF")
	if strings.Contains(got, "THOTH%20Chat%3A") {
		t.Fatalf("issuer/account separator should stay as ':' in otpauth label: %s", got)
	}
	if !strings.HasPrefix(got, "otpauth://totp/THOTH%20Chat:") {
	...
	query := parsed.Query()
	if query.Get("secret") != "ABCDEF" || query.Get("issuer") != "THOTH Chat" || query.Get("digits") != "6" || query.Get("period") != "30" {
