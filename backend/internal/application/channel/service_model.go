...
import (
	...
	"github.com/THOTH-AI/THOTH-Chat/backend/internal/shared/channelconfig"
	"github.com/THOTH-AI/THOTH-Chat/backend/internal/shared/nativetool"
	"go.uber.org/zap"
)
...
func clearAutomaticContextWindow(raw string) (string, bool) {
	...
	mode, ok := payload["_thothContextWindowMode"].(string)
	if !ok || !strings.EqualFold(strings.TrimSpace(mode), "auto") {
		return raw, false
	}

	delete(payload, "_thothContextWindowMode")
