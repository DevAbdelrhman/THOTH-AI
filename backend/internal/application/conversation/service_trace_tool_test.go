...
func TestToolOutputPreviewUsesMCPStructuredContent(t *testing.T) {
	raw := `{"structuredContent":{"results":[{"title":"THOTH Chat 文档","url":"https://example.com/docs"}]}}`
	if got := toolOutputPreview(raw); got != "THOTH Chat 文档 https://example.com/docs" {
...
func TestToolExecutionLedgerNormalizesArguments(t *testing.T) {
	...
	row := model.ToolCall{
		...
		ToolName:   "bing_search",
		Status:     "success",
		InputJSON:  `{"query":"THOTH Chat","count":3}`,
		OutputJSON: `{"content":[{"type":"text","text":"ok"}]}`,
	}
	...
	if _, ok := ledger.lookup("BING_SEARCH", `{"count":3,"query":"THOTH Chat"}`); !ok {
