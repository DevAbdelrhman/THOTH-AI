...
const (
	defaultAppName                      = "THOTH Chat"
	defaultBrandTitle                   = "THOTH Chat"
	defaultBrandShortName               = "THOTH"
	defaultBrandDescription             = "THOTH Chat is a multi-model AI conversation system."
	...
	defaultJWTSecret                    = "thoth-chat-dev-secret"
	defaultDataEncryptionKey            = "thoth-chat-dev-data-encryption-key"
...
func Load() Config {
	...
	return Config{
		...
		PostgresDSN:                  normalizePostgresDSN(envOr("POSTGRES_DSN", yc.Database.Postgres.DSN, "host=127.0.0.1 user=thoth_chat password=thoth_chat_dev_2026 dbname=thoth_chat port=5432 sslmode=disable TimeZone=Asia/Shanghai")),
		...
		SQLitePath:                   envOrPath("SQLITE_PATH", yc.Database.SQLite.Path, "./data/thoth.db", yc.sourceDir),
