...
import (
	...
	sharedsecurity "github.com/THOTH-AI/THOTH-Chat/backend/internal/shared/security"
)
...
func TestLoadNormalizesLegacyPostgresDSNTimeZone(t *testing.T) {
	cleanupConfigEnv(t)
	chdir(t, t.TempDir())
	t.Setenv("POSTGRES_DSN", "postgres://thoth_chat:secret%2Fvalue@postgres:5432/thoth_chat?sslmode=disable&TimeZone=Asia%2FShanghai")

	cfg := Load()
	if cfg.PostgresDSN != "postgres://thoth_chat:secret%2Fvalue@postgres:5432/thoth_chat?sslmode=disable&TimeZone=Asia/Shanghai" {
...
func TestNormalizePostgresDSNTimeZone(t *testing.T) {
	tests := []struct {
		...
		{
			name: "key value",
			dsn:  "host=postgres user=thoth password=secret%2Fvalue dbname=thoth sslmode=disable TimeZone=Asia%2FShanghai",
			want: "host=postgres user=thoth password=secret%2Fvalue dbname=thoth sslmode=disable TimeZone=Asia/Shanghai",
		},
		{
			name: "already normalized",
			dsn:  "host=postgres user=thoth dbname=thoth sslmode=disable TimeZone=Asia/Shanghai",
			want: "host=postgres user=thoth dbname=thoth sslmode=disable TimeZone=Asia/Shanghai",
		},
		{
			name: "other percent encoded fields unchanged",
			dsn:  "postgres://user:pass@postgres:5432/db?application_name=THOTH%2FChat&sslmode=disable",
			want: "postgres://user:pass@postgres:5432/db?application_name=THOTH%2FChat&sslmode=disable",
		},
	}
