...
func TestUpdateFieldsKeepsLastSuperAdminProtected(t *testing.T) {
	...
	if dsn == "" {
		t.Skip("set THOTH_TEST_DATABASE_DSN to run PostgreSQL repository guard integration test")
	}
...
func openUserRepositoryIntegrationDB(t *testing.T, dsn string) (*gorm.DB, func()) {
	...
	schemaName := fmt.Sprintf("thoth_test_superadmin_guard_%d", time.Now().UnixNano())
