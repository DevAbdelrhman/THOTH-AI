...
func TestEnsurePostgresVectorColumnPreservesLegacyVectors(t *testing.T) {
	...
	if dsn == "" {
		t.Skip("set THOTH_TEST_DATABASE_DSN to run PostgreSQL vector migration integration test")
	}
	...
	schemaName := fmt.Sprintf("thoth_test_vector_migration_%d", time.Now().UnixNano())
