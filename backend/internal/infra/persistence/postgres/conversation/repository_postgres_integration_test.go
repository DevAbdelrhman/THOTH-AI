...
func TestSearchMessageChunksFiltersPostgresBranchBeforeTopK(t *testing.T) {
	...
	if dsn == "" {
		t.Skip("set THOTH_TEST_DATABASE_DSN to run PostgreSQL branch-scoped vector integration test")
	}
...
func openConversationPostgresIntegrationDB(t *testing.T, dsn string) (*gorm.DB, func()) {
	...
	schemaName := fmt.Sprintf("thoth_test_conversation_scope_%d", time.Now().UnixNano())
