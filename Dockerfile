...
RUN --mount=type=cache,id=pnpm-store,target=/pnpm/store \
    ...
    && pnpm install --frozen-lockfile --prefer-offline --filter @thoth/web
...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    && CGO_ENABLED=1 \
       go build -trimpath \
       -ldflags="-s -w -X github.com/THOTH-AI/THOTH-Chat/backend/internal/shared/buildinfo.Version=${VERSION} -X github.com/THOTH-AI/THOTH-Chat/backend/internal/shared/buildinfo.Commit=${GIT_COMMIT} -X github.com/THOTH-AI/THOTH-Chat/backend/internal/shared/buildinfo.BuildTime=${BUILD_TIME}" \
       -o /out/thoth-chat ./cmd/server
...
COPY --from=backend-builder /out/thoth-chat /app/thoth-chat
COPY --from=frontend-builder /src/frontend/out /app/frontend/out
COPY LICENSE NOTICE /app/licenses/THOTH-Chat/
