...
THOTH Chat 前端是基于 Next.js App Router 的管理与对话界面，负责聊天工作区、模型参数配置、文件页、最近会话、用户设置、MCP 工具选择、官方原生工具配置和管理员后台。
...
- `api/`：该业务域自己的接口封装和契约适配类型。HTTP 传输类型来自 `@thoth/api-contract`；跨业务、用户侧通用或基础资源接口放到 `shared/api/`。
...
- `shared/api/*.types.ts` 和 `features/*/api/*.types.ts` 必须从 `@thoth/api-contract` 导入传输类型。
