...
import { AppLogo, ThothLogo } from "@/shared/components/app-logo";
...
import { PoweredByThoth } from "@/shared/components/powered-by-thoth";
    ...
    >
      {placement === "top" ? (
        <>
          ...
          {branding.logoURL ? <PoweredByThoth className="text-[10px]" /> : null}
        </>
      ) : branding.logoURL ? (
        <>
          <AppLogo width={65} height={20} className="h-5 w-auto opacity-75" />
          <span aria-hidden="true" className="h-4 w-px bg-border" />
          <ThothLogo width={65} height={20} className="h-5 w-auto opacity-75" />
        </>
      ) : (
        <ThothLogo width={65} height={20} className="h-5 w-auto opacity-75" />
      )}
