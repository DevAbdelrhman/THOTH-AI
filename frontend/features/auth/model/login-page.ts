...
export const TWO_FACTOR_CHALLENGE_STORAGE_KEY = "thoth-chat:2fa:challenge";
export const TWO_FACTOR_METHODS_STORAGE_KEY = "thoth-chat:2fa:methods";
...
export function providerPKCEStorageKey(slug: string): string {
  return `thoth-chat:oauth:${slug}:pkce_verifier`;
}
...
export function providerAuthBridgeStorageKey(slug: string): string {
  return `thoth-chat:oauth:${slug}:bridge`;
}
