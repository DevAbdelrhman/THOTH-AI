# Contributing to THOTH Chat

Thank you for contributing to THOTH Chat.
...
- Frontend transport types must import from `@thoth/api-contract`. Do not copy generated request or response fields into local interfaces and do not repair generated contracts with `Required<>`.
...
- derive wire types from `@thoth/api-contract` and keep UI state types inside the owning feature
...
- run `pnpm --filter @thoth/web lint`, and run `pnpm build` for routing, dependency, or Next.js changes
